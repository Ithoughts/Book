...
// You must implement this method, which ensures the view's underlying layer is of type CAEAGLLayer
+ (Class)layerClass
{
...
{
    // Called in response to applicationWillResignActive.  The render loop has been stopped,
    //  so we now make sure all OpenGL ES commands complete before
...
//
// This method is called by QCAR when it wishes to render the current frame to the screen.
//
...
- (void)renderFrameQCAR
{
    ...
    if (state.getNumTrackableResults() > 0)
    {
        ...
        QCAR::Vec3F targetSize = imageResult->getTrackable().getSize();
...
/**************************** 建立了之前声明的 defaultFramebuffer 实例变量 ********************************/

- (void)setFramebuffer
{
    // The EAGLContext must be set for each thread that wishes to use it.  Set it the first time this method is called (on the render thread)
    if (context != [EAGLContext currentContext]) {
        [EAGLContext setCurrentContext:context];
    }
