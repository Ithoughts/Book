...
- (void) addLoadingView
{
    ...
    UIActivityIndicatorView * anActivityIndicator = [[[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:
...
- (CGRect) rectForStatusLabelWithString:(NSString *)aString
{
    ...
    CGRect aRect = CGRectMake(self.view.frame.size.width / 2 - statusLabelWidth / 2, self.view.frame.size.height - statusLabelHeight - statusLabelBottomMargin, statusLabelWidth, statusLabelHeight);
...
- (void)addCloseButton
{
    ...
    CGRect aRect = CGRectMake(self.view.frame.size.width - closeButtonImage.size.width, 0, closeButtonImage.size.width, closeButtonImage.size.height);
...
- (void) handleViewRotation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    int smallerSize = MIN(screenRect.size.width, screenRect.size.height);
    int largerSize = MAX(screenRect.size.width, screenRect.size.height);
    
    CGRect overlayRect = screenRect;
    
    if (interfaceOrientation == UIInterfaceOrientationLandscapeLeft || interfaceOrientation == UIInterfaceOrientationLandscapeRight)
    {
        overlayRect.size.width = largerSize;
        overlayRect.size.height = smallerSize;
    }
